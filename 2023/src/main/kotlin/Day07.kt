...
class Day07 {
    ...
    data class CrazyHand(val cards: String, val type: HandType) : Comparable<CrazyHand> {
        // A, K, Q, J, T, 9, 8, 7, 6, 5, 4, 3, or 2
        val internalCards = cards
            .replace("A", "a")
            .replace("K", "b")
        ...
        override operator fun compareTo(other: CrazyHand): Int {
            return when {
                this.type > other.type -> 1
                this.type < other.type -> -1
                else -> other.internalCards.compareTo(this.internalCards)
    ...
    fun parseHandsAndBids(input: String) = input
        .trim()
        .split("\n")
        .map { it.split(" ") }
    ...
    fun parseCrazyHandsAndBids(input: String) = input
        .trim()
        .split("\n")
        .map { it.split(" ") }
        .map { (handString, bidString) ->
            CrazyHand(handString, classifyCrazyHandString(handString)) to bidString.toLong()
        }
    ...
    fun solution02(input: String): Long {
        return parseCrazyHandsAndBids(input).sortedBy { it.first }.mapIndexed { index, (_, bid) ->
            (index + 1) * bid
        }.sum()
    }
}
